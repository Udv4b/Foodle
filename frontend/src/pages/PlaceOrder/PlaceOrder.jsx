  ...
  const placeOrder = async (event) => {
    ...
    food_list.map((item) => {
      if (cartItems[item._id] > 0) {
        let itemInfo = { ...item }; // Use spread to avoid mutating original food_list
    ...
    try {
      ...
      // Fix: check for 'success' (you had 'succes')
      if (response.data.success) {
        const { order } = response.data;
        initPay(order); // Call helper function to open Razorpay modal
      } else {
